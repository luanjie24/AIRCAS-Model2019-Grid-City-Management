...
random.shuffle(Mon)
'''
Mon = np.linspace(delay+1,num_Data,num_Data-delay)
#shuffle data
index=np.arange(num_Data-delay)
...
train_set_x=X[:trLen,:] 
train_set_y=y[:trLen] #所有站点的前trLen个数据
test_set_x = X[trLen:,:]
test_set_y=y[trLen:]
...
rnn_out = LSTM(50, return_sequences=True,consume_less = 'gpu')(main_input)
x = LSTM(50,consume_less = 'gpu')(rnn_out)
...
with open(u"输出文件："+file_name, "w", newline="",encoding="utf-8-sig") as datacsv:
    ...
    for i in range(0, num_Data-trLen-delay):
